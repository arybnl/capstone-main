        ...
        function sortHistoryList() {
            ...
            items.sort((a, b) => {
                const dateA = new Date(a.querySelector('.archive-date').textContent);
                const dateB = new Date(b.querySelector('.archive-date').textContent);
        ...
        document.getElementById('archiveYesBtn').addEventListener('click', function() {
            if (archiveId) {
                const item = document.querySelector(`.archive-item[data-id="${archiveId}"]`);
                if (item) item.remove();
                archiveId = null;
